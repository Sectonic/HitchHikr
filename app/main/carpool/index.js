...
export default function Page() {
    ...
    const getData = async () => {
        ...
        const request = await fetch(`${API_URL}/carpool/get?` + new URLSearchParams({ hash: session, type: 'carpooler' }));
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: 'white', paddingHorizontal: 20 }} >
            ...
            <Tabs.Screen
                options={{ headerShown: false }}
            />
            <Text style={{ fontSize: 35, fontWeight: 700 }} >Carpools as <Text style={{ color: COLORS.green }} >Carpooler</Text></Text>
            <TouchableOpacity onPress={() => router.push('/main/carpool/search')}  style={{ paddingVertical: 10, width: '100%', backgroundColor: COLORS.green, borderRadius: 10, marginTop: 15 }}>
                <Text style={{ textAlign: 'center', color: 'white', fontSize: 17, fontWeight: 500 }} >Search for Carpool</Text>
            </TouchableOpacity>
            ...
            <FlatList
                data={carpools}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => router.push('/main/carpool/' + item.id)} style={{ marginTop: 20, width: '100%', borderRadius: 10, borderWidth: 1, borderColor: COLORS.lightGray, flexDirection: 'row', justifyContent: 'space-between', alignItems: item.active ? 'flex-end' : 'center', padding: 20, gap: 20 }} >
                        ...
                        <View style={{ justifyContent: 'center', alignItems: 'center', gap: 10 }} >
                            <Image 
                                ...
                                source={require('../../../assets/carpool.svg')}
                            />
                            <View>
